...
object GatheringNodeBlock : BlockModDirectional("gathering_node", Material.IRON, true) {
    init {
        setBlockUnbreakable()
    }
    ...
    override fun createTileEntity(world: World, state: IBlockState) = GatheringNodeTE()
    ...
    override fun onBlockActivated(
        ...
        worldIn onServer {
            GuiHandler.open(GatheringNodeContainer.NAME, playerIn, pos)
        }
        true
    } else {
        val tile = worldIn.getTileEntity(pos)
...
class GatheringNodeTE : TileModInventoryTickable(1) {

    @Module
    val gatheringHandler = GatheringNodeModule()

    override fun tick() = Unit
}

class GatheringNodeModule :
    ModuleCapability<GatheringCapability>(GatheringCapability.CAP_INSTANCE, GatheringCapability()) {
...
class GatheringNodeContainer(player: EntityPlayer, val te: GatheringNodeTE) : ContainerBase(player) {

    val invPlayer = RPGInventoryContainer.InventoryWrapperPlayer(
    ...
    companion object {
        ...
        init {
            GuiHandler.registerBasicContainer(
                NAME, { player, _, te ->
                    GatheringNodeContainer(player, te as GatheringNodeTE)
                }, { _, container ->
